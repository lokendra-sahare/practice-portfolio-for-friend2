...
export const Contact = () => {
  ...
  const sendEmail = (e) => {
    ...
    emailjs
      .sendForm("service_qvvny2p", "template_a1x301g", formRef.current, {
        publicKey: "dPKhP6LT7UAo_T9K9",
      })
      .then(
        () => {
          setSuccess(true);
          setTimeout(() => setSuccess(false), 2000); // Hide message after 2 seconds
        },
        (error) => {
          setError(true);
          setTimeout(() => setError(false), 2000); // Hide message after 2 seconds
        }
      );
  };
    ...
    >
      <motion.div className="textContainer" variants={variants}>
        ...
        <motion.div className="item" variants={variants}>
          <h2>Mail</h2>
          <span>lokendrasahare@gmail.com</span>
        </motion.div>
        <motion.div className="item" variants={variants}>
          <h2>Address</h2>
      ...
      <motion.div className="formContainer" variants={variants}>
        <motion.div
          className="phoneSvg"
          initial={{ opacity: 1 }}
          whileInView={{ opacity: 0, zIndex: -999 }}
